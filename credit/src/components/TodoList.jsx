...
const TodoList = ({
    ...
    return (
        <div>
            {validate && mytodos ? mytodos?.map((todo) => (
                todo.user === user.email )) && mytodos.length === 0 ? <h4 className="text-xl text-center my-9 font-bold capitalize text-gray-800">no available todo list</h4> : <h2 className="text-xl text-center my-9 font-bold capitalize text-gray-800">Active Tasks</h2> : todos.length === 0 ? <h4 className="text-xl text-center my-9 font-bold capitalize text-gray-800">no available todo list</h4> : <h2 className="text-xl text-center my-9 font-bold capitalize text-gray-800">Active Tasks</h2>}
